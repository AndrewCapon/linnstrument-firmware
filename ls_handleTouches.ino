...
// Used for pre testing note count when touchinfo has not yet been updated 
byte countTouchesForMidiChannel(byte split, byte channel) {
  return noteTouchMapping[split].getMusicalTouchCount(channel);
}
  ...
  if (userFirmwareActive) {
    ...
    if (!isLowRow() &&                                                   // don't perform slide transfers
        (!Split[sensorSplit].sendX ||                                    // if pitch slides are disabled
         ...
         ((Split[sensorSplit].monoMode == monoOff) && (countTouchesForMidiChannel(sensorSplit, col, sensorRow) > 1)))) { // when there are several touches for the same MIDI channel and legato is off
...
void transferFromSameRowCell(byte col) {
  ...
  updateChannelOffsetColRow(sensorSplit, (byte) sensorCell->channel, sensorCol, sensorRow);
}
...
boolean handleXYZupdate() {
  ...
  if (newVelocity) {
    ...
    // Split strum only triggers notes in the other split
      ...
      notenum = cellTransposedNote(sensorSplit);
...
void handleTouchRelease() {
  ...
  // is this cell used for low row functionality
    ...
    // handle release for channel offsets
    handleChannelOffsetRelease(sensorSplit, sensorCell->channel);
